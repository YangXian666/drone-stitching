...
- **Stage A 的設計決定（已定案）**：
  ...
  - **待注意**：只有序列相鄰邊（圖是一棵樹）時，任何旋轉平均的解在數學上都等於
    ...
    θ_ab 的取法已定案為影像中心 Jacobian polar decomposition（見下面「第二個
    座標系陷阱」）。edge→node 的角度正負號要用獨立推導的期望值測試，不能用
...
- **因此失效或待重新驗證的舊結論**：上面「已知的暫緩事項」的「旋轉退化暫緩到
  ...
  重新驗證。
- **第二個座標系陷阱：θ_ab「在哪一點線性化」（2026-09-26）**。這是一個獨立的
  問題，**量級比鏡射 bug 小得多，但偏差同號，會沿鏈累積**，性質也不同：鏡射是
  det=−1 的座標框架錯誤；這個是把投影映射硬當成 Sim(2) 時，「旋轉角在影像哪一點
  量」的隱含假設錯誤。
  - **背景**：`build_pose_graph` 的 `relative_pose` 就是原始 homography，第三行的
    透視項 `g=(h20,h21)` 還在（真實邊約 1e-6～2e-5）。只要 g≠0，某一點 p 的局部
    Jacobian `J(p) = (A·w − u·gᵀ)/w²` 就會隨位置改變，所以「homography 的旋轉角」
    本身取決於在哪一點量。`H[:2,:2]` 在 g≠0 時不是任何一點的局部線性化。現有
    `optimize_pose_graph` 的旋轉殘差 `R_dst@R_rel − R_src` 解出來，隱含用的就是
    「對 `H[:2,:2]` 做 polar decomposition」：fixture 0352/0353 用這個方法算出
    −61.584°，跟上面記錄的「edge-only 收斂 node1 = −61.584°」完全一致。
  - **真實資料比較**（θ_ab = −angle(H)，對照 GimbalYawDegree 差值）：
    - 只看 `tests/fixtures/dji_smoke/` 0352/0353 這一對（BF，40 個 inlier），
      分不出高下：`H[:2,:2]` atan +0.12°、`H[:2,:2]` polar +0.62°、中心 Jacobian
      polar −0.59°。
    - 0350～0361 的 11 條邊（FLANN）差異就很清楚：

      | 方法 | 誤差範圍 | 平均 \|誤差\| | 11 條邊累加 |
      |---|---|---:|---:|
      | `H[:2,:2]` polar（現有最佳化隱含使用） | **11 條全部是正的**，+0.19～+1.87° | 1.00° | +11.0° |
      | `H[:2,:2]` atan(H10,H00) | −1.54～+1.80° | 0.98° | — |
      | 左上角 (0,0) 的 Jacobian polar | 幾乎全正，−0.58～+1.58° | 0.84° | — |
      | **影像中心 Jacobian polar** | −0.22～+0.76° | **0.17°** | +1.0° |

    - 上面「edge-only 鏈走 9 段偏 6.58°」的舊觀測，量級跟這個每條邊約 +1° 的同號
      偏差吻合，但沒有單獨驗證。
  - **合成驗證（機制本身）**：用針孔相機模型（f=2869.7 px，由 DFOV 82.9° 推得；
    高度 100 m）對地平面 z=0 建立精確的平面誘導 homography，相對偏航的真值已知：
    - 沒有傾斜時：4 種方法的誤差都是精確 0（U 型迴轉情境 ψ 39.0→−23.2°、直線
      情境各一組）。這同時獨立驗證了正負號慣例 **θ_ab = −angle(H) = ψ_dst − ψ_src**。
    - 相機 2 相對傾斜 0.1～2°（傾斜方位角每 45° 掃一次）：所有方法的誤差都跟傾斜
      量成線性，但中心 Jacobian 的誤差約只有以角落為基準的方法的 1/6（傾斜 2°、
      |g|≈1.2e-5 時，`H[:2,:2]` polar 0.88°、中心 Jacobian 0.14°）。這跟真實資料
      的平均誤差 1.00° 對 0.17° 在量級上吻合（真實邊的 |g| 在 1e-6～2e-5，換算成
      相對傾斜約 0.9～4°）。
    - **正負號機制**：以角落為基準和以中心為基準量到的角度差，正負號跟著 g 的方向
      走，大約等於 0.5～0.6 × (c × g)（c 是影像中心座標，c × g = c_x·g_y − c_y·g_x）。
      合成資料 8 個方位角全部符合。真實 11 條邊裡有 9 條符合（直線段的比例
      0.52～0.63）；兩個例外（0351→0352、0354→0355）都是相對旋轉大的 U 型迴轉邊，
      一階近似 c × g 沒有考慮旋轉，沒有再深入追。真實邊的 h21 在 11 條邊上全部是
      正的，所以 c × g 大多為正，**這就是偏差一直是正號的原因**。
  - **還沒解釋的部分**：為什麼真實邊的 g 方向一致（隱含 0.9～4° 的相對傾斜），
    但 GimbalPitch 讀數是 −89.9～−90.0°、GimbalRoll 是 0。可能是雲台穩定的殘餘
    誤差、沒有做鏡頭畸變校正（homography 去擬合徑向畸變，可能產生方向一致的 g）、
    或 RANSAC 在海面上的擬合假象。都還沒驗證，不影響這次的決定。
  - **定案：Stage A 的 θ 提取用影像中心 Jacobian 做 polar decomposition**
    （對 2×2 且 det>0 的矩陣，等價於 `atan2(J10−J01, J00+J11)`），
    θ_ab = −angle(J(center))。
  - **Stage D 要記得**：edge 平移殘差用的 `t_rel = H[:2,2]` 同樣忽略了透視項，
    會遇到同一個「在哪一點線性化」的問題。這次先記下來，不處理。

### Stage A：特徵向量下溢 bug 與 loop closure 診斷（2026-09-26）

- **特徵向量下溢 bug（已修正）**：`average_rotations` 最早用 Hermitian 量測矩陣
  W（`W[dst,src] = w·e^{iθ}`）的主特徵向量。權重是 `inlier_count / median`，真實
  範圍約 0.01～11，很不均勻。在這種權重下，W 的主特徵向量會集中在權重最大的邊
  附近，往外指數衰減；鏈夠長時，遠端的分量會衰減到浮點下溢、變成精確的 0，
  `np.angle(0) = 0`，相位整個失去意義。在 52 張資料的純序列鏈上的症狀：line B
  的 0317～0339 連續 23 個 node 誤差「精確 0.00°」（分量模長 0e+00），接著
  0341/0342/0345/0347 出現 ±180° 跳動（分量模長 5e-17～8e-16，只剩捨入雜訊的
  正負號），最大誤差 171°。**不需要雜訊也會發生**：50 節點、無雜訊、權重
  `geomspace(0.01, 11)` 的合成鏈上，plain W 有 46 個 node 錯（最大 180°）。
- **修正**：改用 connection Laplacian `L = D − W` 的最小特徵向量。一致（無雜訊）
  的量測下，這個特徵向量精確等於真解，而且每個分量模長相同，跟權重分布、圖的
  度數都無關，所以不會集中、不會下溢。真實資料上 `D − W` 和正規化版本
  `D^{-1/2} W D^{-1/2}` 的結果到 0.01° 都相同，分量模長比都在 0.99～1.0。已用
  TDD 補上 `test_noise_free_long_chain_with_heterogeneous_weights_is_recovered_exactly`：
  先確認舊版紅燈（node1 回傳精確 `0.0°`，期望 `7.3°`），再改成 `D − W` 轉綠。
- **修正是純加固，沒有引入新行為**：0350～0361 短鏈的 12 個 node 在修正前後完全
  相同（最大誤差都是 0.98°），因為那條鏈短、權重接近均勻，本來就不會觸發下溢。
  52 張的數字也跟修正前用診斷腳本算的 `D − W` 結果逐位相同。
- **教訓：兩輪前是「巧合選對、理由錯誤」地繞過這件事**。Stage A 第一次實作時就
  用了 `D − W`，但給的理由是「plain W 在度數不均的鏈上即使無雜訊也有偏差」——
  這個理由是錯的（無雜訊時，W 主特徵向量的相位在精確算術下是對的，度數只影響
  最後被丟掉的模長；而且在單位圓上 `D` 是常數項，兩者是同一問題的兩種鬆弛）。
  mutation 檢查抓到這個理由錯誤後，就改回了 plain W，而當時 27 條測試全部用均勻
  權重、短鏈，沒有一條能區分兩者。真正的差異在「精確算術」和「浮點」之間：
  理論上的相位正確，擋不住實際上的分量下溢。**兩個具體教訓**：(1) mutation 檢查
  證明「這個理由錯」，不等於證明「這個選擇錯」——推翻理由之後，應該去找有沒有
  別的理由支持原本的選擇，而不是直接改回去；(2) 合成測試的參數範圍要涵蓋真實
  資料的範圍（這次是權重跨 3 個數量級、鏈長 50），否則測試綠燈只代表在沒遇過的
  範圍外沒被檢驗過。
- **loop closure 診斷（52 張，GPS 距離 < 40 m 的 184 對，其中 62 對跨航線；
  SIFT + FLANN，約 21 分鐘 CPU）**，逐 node 對照 GimbalYawDegree（各連通分量以
  最小 index 為 gauge）：

  | | 連通分量 | line B 誤差範圍 | line C 誤差範圍 | 最大誤差 |
  |---|---:|---:|---:|---:|
  | 純序列（50 對，缺 0304–0317） | 2（line A 斷開） | 12.4° | 1.2° | 15.5° |
  | GPS R<40m（184 對） | 1 | **13.5°** | 0.5° | 10.4° |

  **核心發現：loop closure 解決了連通性，但沒有解決系統性偏差**。line B 沿航線
  的漂移在加上跨航線 loop 之後沒有變小；最大誤差下降只是因為 gauge 基準點移動了。
  - **驗證 1（loop 內誤差自我抵銷）**：184 條邊彼此高度一致——對解出來的角度，
    邊殘差中位數 0.26°、90 百分位 0.96°——但相對 GimbalYawDegree 有同號偏差：
    序列邊平均 +0.26°、同航線全部邊 +0.44°、跨航線 B–C +2.63°、A–B −1.41°。
    如果每條邊的偏差可以寫成兩端 node 某個量的差（`b_ij = f(j) − f(i)`，例如
    跟兩張影像在世界座標的位移成正比），沿任何路徑加總都只跟起點終點有關，
    繞任何一個 loop 一圈的總和精確為 0——loop 看不見它，旋轉平均只會把它當成
    一致的訊號接受下來。同航線跳一張的邊（i, i+2）偏差大約是相鄰邊的兩倍，跟這個
    形式吻合。
  - **驗證 2（GPS 航跡方位角，獨立交叉驗證）**：只用平移、不用任何旋轉估計。
    line B 的 GPS 航跡方位角全程固定在 83.0° ± 0.2°；用 homography 量出每張影像
    座標系裡的前進方向，兩者相減得到「影像朝向」。line B 34 條邊的影像朝向一直
    落在約 68～70°（±2° 單邊雜訊），**沒有 12° 的趨勢**，GimbalYawDegree 讀數固定
    70.8°。所以影像本身沒有轉，漂移來自邊的旋轉估計，不是 GimbalYawDegree 讀錯。
  - **對配對策略的結論**：值得為了連通性換掉 `sequential_pairs()`（把 line A 接回
    主圖、單一壞邊有冗餘可抵），但不要期待 loop 修正旋轉漂移。這個漂移約每條邊
    0.3°，700 張、航線更長時會更大，必須靠絕對參考在 Stage C/D 修正。除了
    GimbalYawDegree，「GPS 航跡方位角 − 影像內前進方向」是第二個候選：不依賴
    DJI 專屬 XMP 欄位、不累積漂移，跟 gimbal yaw 差一個約 1.8° 的常數，單邊雜訊
...
- **核對邊界時查出的三個牴觸，以及處理方式**：
  ...
  3. **閾值不能用 gimbal 對照量出的數字定**：之前的「常數偏移約 1.8°、每條邊雜訊約
     ...
     實際分佈、確認有道理的切點後才寫進這裡定案，不憑感覺抓數字。**已定案為
     5 m**，見下面「最小 GPS 位移閾值與 Stage C 漂移發現」。
...
### 最小 GPS 位移閾值與 Stage C 漂移發現（2026-09-26）

- **診斷資料**：52 張、184 對（GPS 距離 < 40 m）的既有配對結果，只讀 EXIF
  經緯度，不用任何 DJI XMP 欄位。每條邊算兩個量：ppm 比值（homography 量到的
  中心位移 px ÷ GPS 位移 m），以及 `β − α`（北向上像素座標系的 GPS 位移方向，
  減去 src 影像座標系裡的前進方向，也就是 src 影像的絕對航向）。航向雜訊用
  「同一個 src node 出發的各邊之間 `β − α` 的離散程度」量，這樣不受 Stage A
  漂移影響：

  | GPS 距離 | n | ppm 中位數 | 相對離散（MAD） | 同 src 航向雜訊 中位數 / p90 |
  |---|---:|---:|---:|---|
  | [0, 5) m | 1 | 58.80 | +110% | — |
  | [5, 10) m | 2 | 29.87 | 6.5% | 5.06° / 6.51° |
  | [12, 15) m | 48 | 28.15 | 2.6% | 1.75° / 5.85° |
  | [15, 20) m | 1 | 26.31 | — | 1.13° |
  | [20, 25) m | 3 | 29.24 | 4.3% | 1.01° / 1.27° |
  | [25, 30) m | 75 | 27.84 | 2.8% | 2.59° / 5.74° |
  | [30, 35) m | 23 | 27.65 | 8.6% | 1.29° / 3.93° |
  | [35, 40) m | 31 | 28.75 | 3.7% | 0.84° / 4.09° |

  12 m 以下只有三條邊：0361→0362（1.64 m，ppm 58.8，高 110%，明顯壞掉）、
  0351→0352（8.37 m，ppm +5.6%）、0353→0354（9.69 m，ppm +7.4%）。後兩條都是
  U 型迴轉的弱邊（41、47 inlier），ppm 仍在 12～15 m 區間的範圍（24.6～36.5）內，
  航向雜訊較大，但跟低 inlier 混在一起，不能單純歸因於距離短。全部 184 條邊的
  ppm 中位數是 28.047 px/m。
- **定案：Stage B/C 共用的最小 GPS 位移閾值 = 5 m。這是模型推導的內插值，不是
  資料直接鎖定的**：
  - 資料本身鎖不住切點：1.64 m 到 8.37 m 之間沒有任何邊，這個區間裡的任何閾值在
    這批資料上結果都完全相同。
  - 模型：12～15 m 區間 13.5 m 處 ppm 相對離散 2.6%，推得典型 GPS 位移誤差約
    **0.35 m**；航向雜訊預測約 0.35/d 弧度。13.5 m 處預測 1.5°，實測同 src 航向
    雜訊 1.75°——模型跟另一個獨立的量吻合，這是這個模型唯一的驗證。
  - 用這個模型，5 m 處航向雜訊約 4°、ppm 誤差約 7%，所以取 5 m。1.64 m 處模型
    預測 ppm 誤差約 21%，實測 110% 是約 5 倍標準差，可能還混了航線末端懸停時的
    時間戳不同步。
  - **10 月必須重新驗證**：700 張的新資料集如果有更多短邊（例如懸停、轉彎、
    重疊率更高），直接看 1～10 m 區間的實際分佈，確認或修正這個值。
- **發現：Stage C 的「每連通分量一個 δ」只能消掉整體偏移，消不掉沿航線的漂移**。
  以全域平均 δ 為準，各邊 `δ_ij` 的離散中位數在每個距離區間都是 5～12°，遠大於
  同 src 航向雜訊（約 1～2°）。原因是 `δ_ij = β_ij − α_ij − θ_i` 裡的 θ_i 帶著
  Stage A 沿 line B 約 13° 的系統性漂移（見「Stage A：特徵向量下溢 bug 與 loop
  closure 診斷」）。對齊之後，航向仍可能偏到約 ±7°。
- **決定：Stage C 維持每連通分量一個 δ**（只負責把 Stage A 的 gauge 對齊到北向
  上座標系）。**逐節點的漂移留給 Stage D 處理。**
- **Stage D 的設計輸入（先記下，還沒展開設計）**：每條邊的 `β − α` 本身就是 src
  node 絕對航向的一次獨立量測——不累積漂移、單邊雜訊約 1～2°（隨距離約 0.35/d
  弧度），而且只用 EXIF 經緯度和影像，完全符合最小可行版本的範圍決定。Stage D 用它
  當每個 node 的航向錨點，**概念上取代原本 YawAnchor 的角色**（YawAnchor 依賴
  GimbalYawDegree，最小可行版本不能用）。可以沿用的既有結論：同 src 多條邊的
  `β − α` 可以先用圓周平均合併；權重可以隨 GPS 距離增加；短於 5 m 的邊不參與。

### Stage D 設計定案（2026-09-26）

- **edge 項：每條邊抽 K 個 RANSAC inlier 對應點，殘差 `pose_i(x_src) − pose_j(x_dst)`，
  不做任何 homography 線性化**。K 先用 30，合理性在測試裡驗證。理由不只是「較不依賴
  近似」：這條除錯鏈反覆出現的陷阱——鏡射 bug、線性化參考點偏差、`H[:2,2]` 忽略透視
  項——根源都是「把非線性 homography 硬塞進某一點的線性近似」。只換一個線性化的點
  （例如改用影像中心）還是同一類問題；直接用對應點才是跳脫整個問題類別。
  - **K 大於實際 inlier 數**：取 `min(K, inlier_count)`；inlier 少於 2 個的邊跳過並回報
    （至少 2 點才能約束旋轉）。
  - **抽樣方式（確定性、可寫成斷言）**：n ≤ K 時回傳全部索引（原始順序）；否則做最遠點
    抽樣——第一點是離所有 inlier src 點重心最遠者，之後每步選「到已選集合的最近距離」
    最大者，同距離都取索引最小者，依被選順序回傳。讓點盡量分散、旋轉槓桿最大，不用亂數。
- **航向錨點：加入，逐邊計算 `h_ij = β_ij − α_ij`，用資料推導的反變異數連續權重
  `w = 1/(σ₀² + k·cos²α)`，σ₀ ≈ 0.5°、k ≈ 57 deg²，沒有二元門檻**。
  - **α 的相位已用真實資料驗證**：α = `atan2(v_y, v_x)`（影像像素，y 朝下），影像縱軸在
    ±90°。已知同航線的連續邊落在 α ≈ −76～−85°（`sin²α` 0.84～0.997，line B 偏約 14°
    是相機朝向跟航跡的夾角）；已知跨航線邊 `sin²α` 中位數約 0.2，但斜向的可以高到 0.765
    ——所以不用單純的 `sin²α`，它會給斜向跨航線邊將近跟同航線邊一樣的權重。
  - **一度錯誤的想法、已更正**：曾考慮用 `cos(β − α)` 當權重。但 `β − α` 本身就是
    node 的航向 φ_i（Stage C 的 `β = φ_i + α`），同一個 node 所有邊的值都約略相同，
    拿它加權等於依羅盤方位角加權，分不出跨航線邊。
  - **參數怎麼來**：對每條邊，算 `β − α` 跟「同一個 src node 裡明確沿航跡的邊
    （`sin²α > 0.9`）的圓周平均」的差。依 `sin²α` 分區的偏差中位數：[0.9,1] 0.49°
    （n=107）、[0.7,0.9) 3.36°、[0.5,0.7) 3.62°（n=3）、[0.3,0.5) 6.10°、
    [0.1,0.3) 6.14°、[0,0.1) 7.39°。模型 `σ² = σ₀² + k·cos²α` 預測 3.4／4.8／5.9／
    6.8／7.4°，跟實測吻合。
  - **跟 5 m 閾值同性質：是資料推導出來的，不是理論值，10 月要用新資料重新驗證。**
  - **限制 1**：參考值本身是用沿航跡邊算的，第一區的 0.49° 有部分是自我比對，可能
    低估了 σ₀（也就高估了同航線邊的權重）。
  - **限制 2**：各區間有正負號的平均值正負交錯（+0.00、−4.92、+2.11、+0.75、−4.68、
    +0.81°），代表跨航線邊在這個量測下是「離散變大」，不是固定方向的偏差。**這跟先前
    對照 GimbalYawDegree 量到的 B–C 跨航線 +2.63°（有方向性的常數偏移）不是同一種
    現象**，兩者的參考基準也不同，記錄和引用時不要混為一談。
- **全域 κ：加入，乘在 GPS 目標上，範圍 ±7.7%**（撞到邊界要回報）。
  - 依據：ppm 中位數 28.035（183 條 ≥ 5 m 的邊）的 bootstrap 標準誤 0.48%、99.9% 區間
    −1.36%～+1.77%；樣本數 ≥ 23 的距離區間，中位數偏離全域 −1.37%～+2.56%。取 3 倍
    子集間最大偏離 = ±7.7%。
  - **方法論教訓（可類推到未來類似的校準情境）**：「單一邊有多吵」（每邊離散度，整體
    MAD 3.47%、30～35 m 區間 8.6%）、「183 條邊中位數本身的不確定度」（0.48%）、
    「不同子集之間的系統性差異」（最多 2.56%）是三個不同尺度的量，不能混用。κ 修正的是
    中位數，所以它的範圍要看後兩者；如果拿每邊離散度的倍數當範圍（例如 2 × 8.6% =
    ±17%），會把單邊雜訊誤當成估計值可能錯多少。
- **有界修正只當護欄，不當正則化**：真正的正則化靠 GPS 項、航向項的權重；硬邊界只攔
  嚴重失敗，每次撞到都記成診斷訊號（先前 scale 卡在 0.80 就是被邊界藏起來的症狀）。
  - `|Δφ| ≤ 20°`：1.5 × Stage C 真實資料上最大的航向殘差（13.2°）。
  - `|Δm| ≤ 5 m`：約 13 × σ_gps。兩者都相對 Stage C 的初值。
- **其他數值細節（依據）**：
  - σ_edge：每條邊自己的 inlier 重投影誤差 RMS（逐邊、資料推導），下限 0.1 px（SIFT
    關鍵點定位精度的量級），避免合成完美資料除以 0。
  - MAD→σ：σ₀、k 與 0.35 m 都是用「中位數絕對偏差」量的，換成標準差乘 1.4826。
    σ_gps = 0.35 × 1.4826 / √2 ≈ 0.37 m（0.35 m 是兩點位移的誤差，換成單點位置除 √2）。
  - 穩健損失：scipy `least_squares` 的 `loss` 只能一體套用在所有殘差上，所以 Huber
    （f_scale = 1.345，常態下 95% 效率的標準常數）同時作用在 edge、GPS、航向三項，
    三項都先除以各自的 σ。
- 參數化：每個 node `(φ_i, m_i)`，m_i 是影像中心在北向上座標系的位置，
  `pose_i(x) = R(φ_i)(x − c_i) + m_i`，scale 固定為 1；加上全域 κ。完整測試清單
  （D1～D14）見 `tests/test_refinement.py`。

### 範圍調整：GimbalYawDegree 以可選弱錨點回到 Stage A；Stage D 求解器（2026-09-26）

- **範圍調整**：使用者決定把 GimbalYawDegree 以**可選的弱權重錨點**補回 Stage A，
  用來壓制「繞 loop 會自我抵銷、但沿航線累積」的旋轉漂移。**這修改了「最小可行版本範圍
  決定」**：Stage A 可以吃 GimbalYawDegree，但它是可選輸入——沒有這個欄位時 Stage A 的
  行為跟之前完全相同，不依賴 DJI metadata 的路徑保留。Stage B／C 不變；Stage D 仍不使用
  GimbalYawDegree（它的航向錨點仍是 GPS 衍生的 `β − α`）。
- **做法**：譜方法不變。有錨點時加一個內部「地面節點」，每個錨點是一條「地面 → node」、
  角度 = 錨點值、權重 = `anchor_weight` 的邊；gauge 因此變成絕對座標（北向上像素座標系
  裡 node 角度 = 羅盤方位角，Stage C 已推導過），輸出不含地面節點。
- **真實資料驗證（唯讀診斷，52 張、184 對）**，評估指標刻意**不依賴 gimbal**：經 Stage C
  對齊後，跟同航線 `β − α`（只用 GPS 加 homography 平移）比的殘差。edge 權重是
  inlier/median（中位數 1.0）：

  | 錨點權重 | line B 殘差 | 斜率 | 全部 node \|殘差\| 中位數 / p90 |
  |---|---|---:|---|
  | 無 | +10.7° → −5.3° | −0.496°/node | 5.28° / 9.20° |
  | 0.01 | +9.7° → −4.4° | −0.433 | 4.48° / 8.30° |
  | 0.1 | +6.0° → −1.4° | −0.205 | 2.32° / 5.27° |
  | **1** | **+2.8° → −0.3°** | **−0.033** | **0.74° / 3.44°** |
  | 10 | +2.3° → −0.3° | −0.010 | 0.90° / 3.42° |

  **錨點權重取 1**：漂移斜率在 w = 1 時降到原本的 7%，再加大幾乎沒有改善（w = 10 的中位數
  反而略升）。這是用這批資料量出來的，跟 5 m 閾值同性質，10 月要重新驗證。
- **這不會解決 Stage D 的 D2b／D14**：這兩條是 Stage D 的合成測試，完全不經過 Stage A——
  Stage C 初值就是精確真值，偏差直接注入在 Stage D 的 edge 對應點上，卡住的原因是最佳解
  本身有衝突（有偏差的 edge vs 航向錨點），大量殘差落在 Huber 線性區。真實資料上如果漂移
  來自 homography／對應點本身（根因未確認），Stage D 裡照樣會有這種衝突，所以 Stage A
  錨點和 D2b／D14 的求解問題要分開處理。
- **Stage D 第一次實作的求解器收斂失敗（診斷紀錄）**：原設計（trf ＋ Huber ＋ 邊界，
  x_scale="jac"）在 8 個測試裡跑到最大評估次數（status 0），全套 205 秒。殘差模型本身是
  對的（線性損失、無邊界時精確收斂）。不是 σ_edge 地板造成的（加 ±0.5 px 雜訊、σ_edge
  升到 0.707 px 仍然不收斂）。三組對照實驗（8 個失敗情境，各 90 秒上限）：
  - 自己寫 IRLS ＋ dogbox：2/8；參數改像素當量（φ × 2535、κ × 1000）：0/8，跟原設計幾乎
    一模一樣，所以參數尺度不是原因。
  - **兩階段（先線性損失、無邊界，再 Huber ＋ dogbox 邊界，從第一階段的解出發）：5/8**，
    漂移、平移、三個邊界測試都精確通過。**決定採用**，這是通用的求解器策略，跟要不要用
    yaw 無關。
  - 仍失敗：D2b、D14（都注入了 edge 旋轉偏差，最佳解有衝突）。
- **D3b 的測試寫錯了**：它比較相鄰 node 的「位移向量」，但整塊只要有一點全域旋轉，位移
  向量就跟著轉。真正的最佳解在這個情境有 0.29° 的全域旋轉（在 373 px 的邊上就是 1.9 px），
  但剛體不變量（相鄰距離）的誤差是 0.000 px。改成用相鄰距離與相鄰位移夾角來測。
  **這也是一個模型性質**：GPS 雜訊（這裡每個 node ±0.37 m 交錯）讓每條邊的 β 各偏約 ±3°，
  航向錨點會把整塊影像拉出一個小幅的全域旋轉。

### Stage D 改為只精修位置；GPS 衍生航向錨點補進 Stage A（2026-09-26）

- **發現：Stage D 的 edge 權重只描述隨機雜訊，會把 Stage A 錨點消掉的漂移拉回來**。
  兩階段求解修好收斂後剩下 D2b、D4、D14 失敗。D2b 的起點就是精確真值（等於「完美的
  Stage A」），精修後航向卻偏了約 2°，而且有沒有航向錨點項結果都一樣（1.99995° 對
  1.99994°）；D4 用 Huber 跟線性損失也一樣（7.26°）。原因：σ_edge 取自重投影誤差 RMS，
  每條 edge 因此宣稱的相對旋轉精度是 σ_edge /(r·√(2K))——合成資料（σ 地板 0.1 px）
  0.0005°，**真實邊 0352→0353 是 0.0088°（σ_edge 1.69 px）、0357→0358 是 0.0069°
  （1.44 px）**——但真實的每邊系統性旋轉偏差是 0.26～0.44°（先前對照 gimbal 量到的
  序列邊／同航線邊平均），**過度自信 30～60 倍**。航向錨點（σ 約 2.4°）完全拉不動，
  Huber 也無效（偏差是系統性的小量，不是離群值）。D2b 注入的 0.4°/邊落在真實範圍內，
  不是極端情境，所以不調小它的偏差量級。
  - 釐清一個容易混淆的地方：Stage A 加錨點後的「0.74° 中位殘差」是 **Stage A 輸出**
    跟 GPS 衍生航向的距離（node 層級）；D2b 的 0.4° 是 **每條 edge 自己的量測偏差**
    （Stage D 的輸入）。Stage A 錨點不會改變 edge 量測，Stage D 用的是同一批
    homography／inlier 點，所以錨點不會讓這個衝突消失。
- **決定（選項 c）：Stage D 不再優化航向**。航向直接採用 Stage A＋C 的結果（加 gimbal
  錨點後中位殘差 0.74°），Stage D 只精修位置 m_i 與全域 κ。理由：讓 Stage D 去「優化」
  一個已經比 edge 聲稱精度更可信的量，只會把結果拉回較差的狀態（D2b 展示的現象）；
  這也跟「有界修正只當護欄、不當正則化」的原則一致。
  - **edge 項的角色變成「只約束相對平移」**：φ 固定後，每個對應點的殘差是
    `(m_i − m_j) − d_k`，`d_k = R_j(x_d − c_j) − R_i(x_s − c_i)` 是已知常數，K 個點的
    最小平方解是 `m_j − m_i = mean(d_k)`。仍然完全不線性化 homography。如果航向有誤差
    δφ，平移估計會帶約 `δφ × |抽樣點重心 − 影像中心|` 的誤差（0.74° × 約 200 px ≈ 2.6 px）。
  - 整個問題對 (m, κ) 變成線性（GPS 殘差 `m_i − κ·p_i` 也是線性），保留兩階段求解
    （Huber ＋ 邊界）。參數從每 node 3 個縮成 2 個加 κ。
  - 移除：航向錨點項、`heading_term`、`heading_edge_weight`、`MAX_HEADING_CHANGE_DEG`、
    `BoundHits.heading`。測試：刪 D2a、D2b、D5b、D10（測的正是被移除的功能）；重寫
    D3b（航向固定後不會再有全域旋轉，位移向量本身也要對）、D4（改比位置誤差）、
    D14（容差改成閉式上界：偏差 × 抽樣點重心差）；新增「航向逐位元原樣傳出」與
    「位置等於獨立組出的線性最小平方解」兩條。
- **這個選擇的後果，以及當場處理掉的方式**：Stage D 從此不修航向，沒有 GimbalYawDegree
  的路徑（不依賴 DJI metadata 的路徑）原本會留著 Stage C 之後約 ±7° 的漂移。**當場評估
  並決定這次一起做完：把 GPS 衍生的 `β − α` 當航向錨點補進 Stage A**。
  - **沒有循環依賴**：`h_ij = β_ij − α_ij` 的 β 只要兩個 EXIF 經緯度點（連 ppm 都不用），
    α 只要 homography，都不依賴 Stage A 的角度。每條邊正反各一個觀測（dst 那端用
    `H·c`）；184 對產生 366 個觀測，52 個 node 全部覆蓋。
  - **重用現有的 `heading_anchors` 介面**：每個 node 的多個觀測先合併成一個值再傳入
    （另一種「每個觀測各自一條地面邊」效果沒有更好，還要擴充 API）。
  - **真實資料結果**（診斷，對照 GimbalYawDegree、扣掉一個常數偏移）：

    | 版本 | line B 斜率 | line B 頭尾差 | 中位數 | p90 | 最大 |
    |---|---:|---:|---:|---:|---:|
    | 無錨點 | +0.477°/node | 13.50° | 5.32° | 7.58° | 8.03° |
    | GPS 錨點，只用 `w(α)` | −0.079 | 11.53° | 0.62° | 2.81° | 10.55°（0351） |
    | GPS 錨點＋距離項＋穩健合併 | **−0.047** | **4.72°** | **0.68°** | **2.52°** | 6.16°（0352） |
    | 參考上限：gimbal 錨點 | +0.021 | 1.18° | 0.42° | 1.45° | 2.10° |

    剩下的弱點集中在 U 型迴轉那幾張（最大約 6°）。
  - **兩個修正與依據**：(1) 權重加距離項 `σ² = σ_α(α)² + (σ⊥/d)²`，σ⊥ = 0.174 m——從
    「13.5 m 同航線邊 σ = 1.4826 × 0.5° = 0.74°」反推的跨航跡誤差，不是猜的；(2) 每個
    node 的觀測用 Huber 穩健合併（c = 1.345，標準常數），處理轉彎時 GPS 記錄位置跟曝光
    時刻可能不同步造成的粗差。
  - **錨點權重 w = 1 是中性預設值，不是依賴 gimbal 挑選的**：評估時只比較了 w = 0.1
    和 1，而且是對照 gimbal 看的；如果拿那個對照來「選」w，等於用 gimbal 幫不依賴
    metadata 的路徑挑參數，違反「閾值不能用 gimbal 決定」的原則，而且找不到其他不循環的
    評估量（同航線 `β − α` 現在本身就是錨點來源）。所以 w = 1 的依據只有一個：**中性
    預設值**——一個錨點的份量等於一條典型 edge（edge 權重是 inlier/median，中位數
    1.0），既不刻意放大、也不刻意壓低錨點。上表的 gimbal 對照**只當診斷佐證記錄，
    不當選擇依據**。10 月如果有其他獨立真值（例如地面控制點），
    再重新評估。
- **教訓：5 m 距離閾值不足以排除「短基線＋方向正好沿影像縱軸」的病態情況（0351
  案例）**。0351↔0352 長 8.4 m（高於 5 m 閾值），在 U 型迴轉起點，`h` 偏了 −15.3°，
  但因為方向幾乎正好沿影像縱軸（sin²α = 1.00），`w(α)` 反而給它全部觀測中最高的權重
  （3344），單獨把 0351 的錨點拉偏到 −11.4°；0351 沿航線的其他邊都在 ±0.9° 內。
  **可類推的教訓**：只看單一因素的權重模型（這裡只看幾何方向），會把「幾何上看起來最
  理想、但物理上退化」的觀測當成最可信的。二元閾值只能排除最極端的情況，沒辦法表達
  「越短越不可信」；需要權重本身帶有距離依賴，再加上對粗差的穩健處理，兩者缺一不可
  （上表：只加距離項最大 8.14°、只加穩健合併 6.79°、兩者都加 6.16° 且頭尾差最小）。
...
- [ ] feature-based pipeline
  ...
  - [ ] **分階段架構（取代一次性聯合最佳化，2026-09-26 定案）**——見「已知的
    ...
    - [x] Stage A: `rotation_averaging.py` 譜方法純旋轉平均（含連通分量檢查）——
      ...
      （connection Laplacian `D − W` 的最小特徵向量，各連通分量分開解，最小 index
      為 gauge），28 條新測試（`tests/test_rotation_averaging.py`），187/187 全專案
      綠燈。mutation 檢查：θ 正負號翻轉被 6 條測試抓到、改用 `H[:2,:2]` 被傾斜相機
      測試抓到、拿掉連通分量切分被 2 條測試抓到、改回 plain W 被不均勻權重長鏈測試
      抓到。真實資料對照 GimbalYawDegree：0350～0361 鏈最大誤差 0.98°，沒有 scale
    ...
    - [x] Stage D: `refinement.py`——只精修位置 m_i 與全域 κ（航向逐位元採用 Stage A＋C），
      見「Stage D 改為只精修位置」。edge 用 K=30 個 inlier 對應點（純相對平移約束），
      問題對 (m, κ) 是線性的：殘差 `A·x − b`，A 是稀疏常數矩陣。求解：第一階段稀疏
      `lsqr` 精確解（疊代求解曾在平坦的 κ 方向提早停下）；第二階段 IRLS——每輪重算
      Huber 權重、稀疏精確解加權線性最小平方，護欄用主動集處理。**近似信賴域子問題
      求解器（lsmr）在「一條錯誤 edge」測試卡在 10 px，精確稀疏解是 0.0125 px（線性
      損失 31.6 px）**；稠密精確求解器也能收斂，但 700 張時 Jacobian 約 1.7 GB，不採用。
      收斂判斷：同一輪內位置變化 ≤ 1e-9 px、κ 變化 ≤ 1e-12、且被護欄固定的變數集合
      不變，三者都成立才算 `"converged"`；跑滿 `IRLS_MAX_ROUNDS = 100` 則如實回報
      `"not_converged"`（跟 `optimize_pose_graph` 的 `optimization_status` 同一標準），
      `irls_rounds` 記錄實際輪數；停止規則抽成 `_irls_converged` 並直接測試三個條件。
      Stage D 不再用 scipy `least_squares`，沒有稠密 Jacobian。`tests/test_refinement.py`
      28 條（刪 D2a、D2b、D5b、D10；重寫 D3b、D4、D14；新增航向逐位元傳出、獨立線性
      最小平方、三條 IRLS 收斂狀態、停止規則、未知 loss 驗證），全套 294/294，Stage D
      測試從 206 秒降到約 7 秒。mutation：拿掉 κ（5 條抓到）、拿掉 Huber（3）、抽樣改前
      K 個（2）、疊代用完當收斂（1）、拿掉主動集穩定條件（1，補了停止規則測試後才抓到）、
      換回航向可變的舊版（9）
    - [x] `frame_alignment.gps_heading_anchors` ／ `gps_heading_sigma_rad`：GPS 衍生航向
      錨點（距離項權重＋Huber 合併），給沒有 GimbalYawDegree 的路徑當 Stage A 錨點，
      `GPS_HEADING_ANCHOR_WEIGHT = 1.0`（中性預設值）；σ₀、k 從 `refinement.py` 搬來。
      9 條新測試（合成真值）。mutation：拿掉距離項、拿掉穩健合併、丟掉反方向觀測，
      各被至少一條抓到
